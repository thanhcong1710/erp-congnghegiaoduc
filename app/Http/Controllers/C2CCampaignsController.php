...
class C2CCampaignsController extends Controller
{
    public function list(Request $request)
    {
        ...
        $total = u::first("SELECT count(s.id) AS total 
            FROM c2c_campaigns AS s WHERE $cond");
        
        $list = u::query("SELECT s.*
            FROM c2c_campaigns AS s 
    ...
    {
        $campaign = u::first("SELECT * FROM c2c_campaigns WHERE id = $id");
