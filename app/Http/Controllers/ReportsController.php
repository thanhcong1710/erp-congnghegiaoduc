...
class ReportsController extends Controller
{
    ...
    public function reportBookDelivered(Request $request)
    {
        ...
        $cond = " ((c.class_id IS NOT NULL AND c.class_id > 0) OR c.book_delivered_date IS NOT NULL) AND ((SELECT SUM(charge_amount) FROM tmp_payments WHERE agreement_id = c.agreement_id AND status IN (0, 1) OR a.debt_amount=0) >= 2000000 OR (SELECT SUM(amount) FROM payments WHERE agreement_id = c.agreement_id) >= 2000000) AND s.status > 0 AND s.branch_id IN (" . Auth::user()->getBranchesHasUser() . ")";
