  ...
  <div id="page-report-18">
    <vx-card no-shadow class="rpt-card">
      <div class="mb-6 flex items-center">
        ...
        <div>
          <h3 class="text-lg font-bold uppercase" style="color:#4f46e5; margin:0;">BÁO CÁO CHI TIẾT THEO TỪNG KHÓA TRONG COMBO</h3>
        </div>
      </div>
      <div class="rpt-filter-grid mb-5">
        ...
        <div>
          ...
          <multiselect name="search_school_year" placeholder="Chọn năm học" v-model="searchData.school_year" :options="schoolYearOptions" label="label" :close-on-select="true" :multiple="false" :searchable="false" track-by="id" selectedLabel="" selectLabel="" deselectLabel=""><span slot="noResult">Không tìm thấy</span></multiselect>
        </div>
        <div>
      ...
      <div class="rpt-actions mb-5">
        ...
        <span class="rpt-badge-count">{{ pagination.total | formatNumber }} dòng</span>
      </div>
        ...
        <table class="rpt-table">
          ...
          <tbody>
            <tr v-for="(tr, idx) in contracts" :key="idx" class="rpt-row">
              ...
              <td class="text-center done-cell">{{ tr.done_sessions | formatNumber }}</td>
              <td class="text-center left-cell">{{ tr.left_sessions | formatNumber }}</td>
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        branch_list: [],
        schoolYearOptions: [ {id:'2023',label:'Năm 2023'},{id:'2024',label:'Năm 2024'},{id:'2025',label:'Năm 2025'},{id:'2026',label:'Năm 2026'} ],
    ...
    methods: {
      reset() { this.searchData = { arr_branch:'', branch_id:'', school_year:'', keyword:'', dateRange:'' }; this.getData() },
      getData() {
        const branch_ids = []
        ...
        const data = { branch_id: this.searchData.branch_id, school_year: this.searchData.school_year ? this.searchData.school_year.id : '', keyword: this.searchData.keyword, start_date, end_date, pagination: this.pagination }
        this.$vs.loading()
        axios.p('/api/lms/reports/18', data).then(res => { this.$vs.loading.close(); this.contracts = res.data.list; this.summary = res.data.summary || this.summary; this.pagination = res.data.paging; setTimeout(() => { this.pagination.init = 1 }, 500) }).catch(e => { console.error(e); this.$vs.loading.close() })
      },
