  ...
  <div id="page-report-30">
    <vx-card no-shadow class="rpt-card">
      <div class="mb-6 flex items-center">
        ...
        <div>
          <h3 class="text-lg font-bold uppercase" style="color:#4f46e5; margin:0;">BÁO CÁO TRẢ LƯƠNG SALE THEO TEAM</h3>
        </div>
      </div>
      <!-- Filters -->
        ...
        <table class="rpt-table">
          <thead>
            <tr>
              ...
              <th style="min-width:140px" class="text-right">THƯỞNG TEAM</th>
              <th style="min-width:140px" class="text-right">TỔNG LƯƠNG</th>
            </tr>
          </thead>
          <tbody>
            <tr class="rpt-row" v-for="(row, idx) in datas" :key="idx">
              ...
              <td class="text-right money-cell">{{ fmtMoney(row.thuong_team) }}</td>
              <td class="text-right money-cell" style="font-weight:bold; color:#4f46e5;">{{ fmtMoney(row.tong_luong) }}</td>
            </tr>
            <tr v-if="summary" class="rpt-row bg-gray-100" style="font-weight:bold;">
              ...
              <td class="text-right money-cell">{{ fmtMoney(summary.total_thuong_team) }}</td>
  ...
  export default {
    ...
    data() {
      return {
        ...
        pagination: { cpage: 1, total: 0, limit: 1000, init: 0 },
        searchData: {
    ...
    methods: {
      ...
      getData() {
        ...
        this.$vs.loading()
        axios.p('/api/lms/reports/30', this.buildPayload())
          .then(res => {
      ...
      exportExcel() {
        ...
        window.open(`/api/lms/exports/report30/${keys.join(',')}/${values.join(',')}?token=${localStorage.getItem('accessToken')}`, '_blank')
      },
    },
  }
</script>
...
#page-report-30 { font-family: 'Inter', sans-serif; }
...
.rpt-row td { padding:12px 10px; vertical-align:middle; border:1px solid #e5e7eb; white-space:nowrap; }
