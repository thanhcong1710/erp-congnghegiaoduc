...
<template>
  ...
  <div id="page-roles-list">
    <vx-card no-shadow class="mt-5">
      <h5>BÁO CÁO ĐỐI SOÁT CREVERSE</h5>
            ...
            <table class="vs-table vs-table--tbody-table" style="width: 1800px">
              <thead class="vs-table--thead">
                <tr>
                  ...
                  <th colspan="1" rowspan="1">Ngày thanh toán</th>
                  <th colspan="1" rowspan="1">Tên học sinh</th>
                  <th colspan="1" rowspan="1">Mã LMS</th>
                  <th colspan="1" rowspan="1">Trung tâm</th>
                  <th colspan="1" rowspan="1">Khoá học</th>
                  <th colspan="1" rowspan="1">Trình độ</th>
                  <th colspan="1" rowspan="1">Tổng gói phí</th>
                  <th colspan="1" rowspan="1">Đã trả</th>
                  <th colspan="1" rowspan="1">Loại thanh toán</th>
                  <th colspan="1" rowspan="1">Học phí còn lại</th>
                  <th colspan="1" rowspan="1">Gói phí</th>
                  <th colspan="1" rowspan="1">Thời điểm tính doanh số</th>
                </tr>
              </thead>
              <tr class="tr-values vs-table--tr tr-table-state-null" v-for="(item, index) in datas" :key="index">
                ...
                <td class="td vs-table--td">{{ item.total_fee | formatNumber}}</td>
                <td class="td vs-table--td">{{ item.payment_amount | formatNumber}}</td>
                <td class="td vs-table--td">{{ item.remaining_amount > 0 ? 'Deposit' : (item.payment_amount== item.total_fee ? 'Full Fee' : 'Final Payment')}}</td>
                <td class="td vs-table--td">{{ item.remaining_amount | formatNumber}}</td>
                <td class="td vs-table--td">{{ item.number_of_months}} tháng</td>
                <td class="td vs-table--td">{{ item.report_month}}</td>
              </tr>
            </table>
  ...
  export default {
    ...
    methods: {
      ...
      getData() {
        ...
        this.$vs.loading()
        axios.p('/api/lms/reports/202', data)
          .then((response) => {
      ...
      exportExcel() {
        const startDate = typeof this.searchData.dateRange != 'undefined' && this.searchData.dateRange!='' && this.searchData.dateRange[0] ?`${u.dateToString(this.searchData.dateRange[0])}`:''
        const endDate = typeof this.searchData.dateRange != 'undefined' && this.searchData.dateRange!='' && this.searchData.dateRange[1] ?`${u.dateToString(this.searchData.dateRange[1])}`:''
        var url = `/api/lms/exports/report202/`;
        var ids_branch = "";
        if (this.searchData.arr_branch && this.searchData.arr_branch.length) {
          this.searchData.arr_branch.map(item => {
