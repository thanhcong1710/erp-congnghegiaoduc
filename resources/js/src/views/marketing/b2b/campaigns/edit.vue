    ...
    <vx-card no-shadow class="mt-5">
      <div class="vx-row">
        <div class="mb-6 vx-col md:w-1/3 w-full">
          <label >Đối tác <span class="text-danger"> (*)</span></label>
          <vue-select
                label="title"
                placeholder="Chọn đối tác"
                :options="html.b2b_source.list"
                v-model="campaign.b2b_source"
                :searchable="true"
                language="tv-VN"
                  @input="saveSource"
            ></vue-select>
                ...
                <div class="vs-con-tbody vs-table--tbody " style="min-height: 250px">
                  <table class="vs-table vs-table--tbody-table">
                    <thead class="vs-table--thead">
                      <tr>
                        <!---->
                        <th colspan="1" rowspan="1" style="min-width: 300px">Gói phí</th>
                        <th colspan="1" rowspan="1">Số tiền khuyến mại</th>
                        <th colspan="1" rowspan="1">Số buổi học bổng</th>
...
<script>
  ...
  export default {
    ...
    async created() {
      ...
      await axios.g(`/api/system/b2b/sources`)
        .then(response => {
        this.html.b2b_source.list = response.data
      }),
      this.loadDetail();
    },
    methods: {
      saveTuitionFee(){
        ...
        this.tuition_fees.map(item => {
          if(item.tuition_fee.id == this.html.tuition_fees.item.id){
            check_exit = 1;
          }
        })
        if(!check_exit){
      ...
      deleteTuitionFee(data){
        const ids_tuition = []
        this.tuition_fees.map(item => {
          if(data.tuition_fee.id != item.tuition_fee.id){
      ...
      saveSource(data = null){
        if (data && typeof data === 'object') {
          ...
          this.campaign.b2b_source = data
          this.campaign.b2b_source_id = source_id
        }else{
          this.campaign.b2b_source = ""
          this.campaign.b2b_source_id = ""
        }
      },
      loadDetail(){
        this.$vs.loading();
        axios.g(`/api/marketing/b2b/campaigns/show/${this.$route.params.id}`)
          .then(response => {
          this.$vs.loading.close();
          if(response.data.length !== 0){
            this.campaign = response.data.campaign
            this.tuition_fees = response.data.tuition_fees
            this.campaign.b2b_source = this.html.b2b_source.list.filter(item => item.id == response.data.campaign.b2b_source_id)[0]
          }else{
            this.$router.push({ path: `/marketing/b2b/campaigns` });
          }
        })
      },
      selectDate(date){
      ...
      save() {
        let mess = "";
        let resp = true;
        if (this.campaign.b2b_source_id == "") {
          mess += " - Đối tác không được để trống<br/>";
          resp = false;
        }
        ...
        this.$vs.loading()
        axios.p("/api/marketing/b2b/campaigns/update",{
          ...
          .then((response) => {
            ...
            if (response.data.status) {
              ...
              this.$router.push('/marketing/b2b/campaigns')
            }else{
              this.$vs.notify({
                title: 'Lỗi',
                text: response.data.message,
