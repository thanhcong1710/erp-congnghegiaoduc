    ...
    <vx-card no-shadow class="mt-5">
      <div class="vx-row">
        <div class="mb-6 vx-col md:w-1/3 w-full">
          ...
          <div class=w-full>
            <input type="text" v-model="b2b_source.title" class="vs-inputx vs-input--input normal">
          </div>
        </div>
        <div class="mb-6 vx-col md:w-1/3 w-full">
          <label>Trạng thái</label>
          <div class=w-full>
            <vs-switch v-model="b2b_source.status" color="success"/>
          </div>
        </div>
        <div class="mb-6 vx-col w-full">
          <label>Nội dung</label>
          <div class=w-full>
            <textarea class="vs-inputx vs-input--input normal" v-model="b2b_source.note"></textarea>
          </div>
        </div>
      </div>
  ...
  export default {
    ...
    methods: {
      save() {
        let mess = "";
        let resp = true;
        if (this.b2b_source.title == "") {
          mess += " - Tên đối tác B2B không được để trống<br/>";
          resp = false;
        }
        if (!resp) {
        ...
        this.$vs.loading()
        axios.p("/api/marketing/b2b/sources/add",this.b2b_source)
          .then((response) => {
