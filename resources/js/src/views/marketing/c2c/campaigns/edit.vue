...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      loadDetail(){
        this.$vs.loading();
        axios.g(`/api/marketing/c2c/campaigns/show/${this.$route.params.id}`)
          .then(response => {
          ...
          if(response.data.length !== 0){
            ...
            this.$router.push({ path: `/marketing/c2c/campaigns` });
          }
        })
      },
      selectDate(date){
      ...
      save() {
        let mess = "";
        let resp = true;
        if (this.campaign.c2c_source_id == "") {
        ...
        this.$vs.loading()
        axios.p("/api/marketing/c2c/campaigns/update",{
